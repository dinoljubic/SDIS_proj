...
int socketUDP(char* ip,char* port,char* timeµSec);
...
int main(int argc, char *argv[])
{
    ...
    while(1){
        socketUDP(argv[1],argv[2],argv[3]);
    }
    return 0; 
}
...
int socketUDP(char* ip,char* port,char* timeµSec){
    ...
    printf("-- To \t[\033[1;34m%s\033[0;0m:\033[1;32m%s\033[0;0m] --> %s",ip,port,toSend);
